...
const Login = ({gauMedi, connectWallet, token, account, setToken, setAccount}) => {
    const navigate = useNavigate();

    const handleSubmit = async (e) => {
        e.preventDefault();
        if(account==="") return;
        var res = await gauMedi.methods.patientInfo(account).call()
        if(res.exists){
            setToken('1');
        ...
        res = await gauMedi.methods.doctorInfo(account).call()
        if(res.exists){
            setToken('2');
        ...
        res = await gauMedi.methods.insurerInfo(account).call()
        if(res.exists){
            setToken('3');
    ...
    useEffect(() => {
        if(gauMedi){   
            ...
            if((t!=="" || a!=="") && (a===account || account==='')){
                if(t==="1"){
                    gauMedi.methods.patientInfo(a).call().then((res) => {
                        if(res.exists){
                            setToken(t);
                            setAccount(a);            
                            return navigate('/dashboard')
                    ...
                    gauMedi.methods.doctorInfo(a).call().then((res) => {
                        if(res.exists){
                            setToken(t);
                            setAccount(a);
                            return navigate('/dashboard')
                    ...
                    gauMedi.methods.insurerInfo(a).call().then((res) => {
                        if(res.exists){
                            setToken(t);
                            setAccount(a);
                            return navigate('/dashboard')
